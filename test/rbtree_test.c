...
static void print( util_rbtree_node_t *node, void *data )
{
	printf( "key=%ld,data=%s\n", node->key,(char*) node->data ) ;
}

// free the momery of the node 
static void freenode( util_rbtree_node_t *node )
{
	free( node->data ) ;
	free( node ) ;
}
...
{
	util_rbtree_t *tree = (util_rbtree_t *) malloc ( sizeof(util_rbtree_t) ) ;
